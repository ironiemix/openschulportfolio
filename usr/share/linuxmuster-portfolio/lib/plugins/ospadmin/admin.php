...
class admin_plugin_confmanager extends DokuWiki_Admin_Plugin {
    ...
    var $cnffiles = array('acronyms','entities','interwiki','mime','smileys','user.css');
    ...
    function _change($file,$conf) {
        if (!is_file(DOKU_CONF.$file.'.conf')) return;
        $org = confToHash(DOKU_CONF.$file.'.conf');
        $cnftext = "";
        ...
        if (empty($cnftext)) @unlink(DOKU_CONF.$file.'.local.conf');
        else file_put_contents(DOKU_CONF.$file.'.local.conf',$cnftext);
    ...
    function _readConf($name) {
        ...
        $result = array();
        $cnf = confToHash(DOKU_CONF.$name.'.conf');
        foreach ($cnf as $k => $v) {
            $result[$k] = array($v,0);
        }
        if (!is_file(DOKU_CONF.$name.'.local.conf')) return $result;
        $cnf = confToHash(DOKU_CONF.$name.'.local.conf');
        foreach ($cnf as $k => $v) {
            $result[$k] = array($v,1);
        }
        return $result;
